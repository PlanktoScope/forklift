...
func (e *FSEnv) GetVersionedReposFS() (pallets.PathedFS, error) {
	return e.FS.Sub(VersionedReposDirName)
}

func (e *FSEnv) LoadVersionedRepo(repoPath string) (VersionedRepo, error) {
...
func (e *FSEnv) ListVersionedRepos() ([]VersionedRepo, error) {
	...
	for _, filePath := range files {
		...
		repoMap[repoPath], err = e.LoadVersionedRepo(repoPath)
		if err != nil {
...
func (e *FSEnv) LoadVersionedPkg(cache *FSCache, pkgPath string) (p *VersionedPkg, err error) {
	p = &VersionedPkg{}
	if p.VersionedRepo, err = e.findVersionedRepoOfPkg(pkgPath); err != nil {
		return nil, errors.Wrapf(
...
func (e *FSEnv) findVersionedRepoOfPkg(pkgPath string) (VersionedRepo, error) {
	...
	for repoCandidatePath != "." {
		repo, err := e.LoadVersionedRepo(repoCandidatePath)
		if err == nil {
			return repo, nil
		}
