...
func LoadFSEnv(fsys core.PathedFS, subdirPath string) (e *FSEnv, err error) {
	e = &FSEnv{}
...
func LoadFSEnvContaining(path string) (*FSEnv, error) {
	...
	for {
		if fsEnv, err := LoadFSEnv(
			core.AttachPath(os.DirFS(envCandidatePath), envCandidatePath), ".",
		); err == nil {
			return fsEnv, nil
		}
...
func (e *FSEnv) getReqsFS() (core.PathedFS, error) {
	return e.FS.Sub(ReqsDirName)
}

// FSEnv: Repo Requirements

// GetRepoReqsFS returns the [fs.FS] in the environment which contains repo requirement
// definitions.
func (e *FSEnv) GetRepoReqsFS() (core.PathedFS, error) {
...
// LoadFSRepoReq loads the FSRepoReq from the environment for the repo with the specified
// path.
func (e *FSEnv) LoadFSRepoReq(repoPath string) (r *FSRepoReq, err error) {
	reposFS, err := e.GetRepoReqsFS()
	if err != nil {
		return nil, errors.Wrap(err, "couldn't open directory for repo requirements from environment")
	}
...
// LoadFSRepoReqs loads all FSRepoReqs from the environment matching the specified search
// pattern.
...
func (e *FSEnv) LoadFSRepoReqs(searchPattern string) ([]*FSRepoReq, error) {
	reposFS, err := e.GetRepoReqsFS()
	if err != nil {
		return nil, errors.Wrap(err, "couldn't open directory for repos in local environment")
	}
...
func (e *FSEnv) LoadPkgReq(pkgPath string) (r PkgReq, err error) {
	...
	if err != nil {
		return PkgReq{}, errors.Wrap(
			err, "couldn't open directory for repo requirements from environment",
		)
	}
...
// configurations.
func (e *FSEnv) getDeplsFS() (core.PathedFS, error) {
...
func loadEnvDef(fsys core.PathedFS, filePath string) (EnvDef, error) {
